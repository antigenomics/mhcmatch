...
it. EPIC fits ``expr_lvl`` at **+0.3704** and ``expr_norm`` at **+0.4950** per standard deviation ---
the largest positive coefficient in the model --- so *as fitted, high normal-tissue expression is
rewarded*. That is not a defect: the model was fitted on **will this respond**, and a gene
