...
feature blocks, backed by 660 fitted log-odds cells. This page records what happened when its two
halves were separated and each was asked to justify its parameters against the neoantigen screens,
and what the shipped API now exposes so that the reduction is reproducible from a fresh install.
...
.. note::
   ...
   Nothing on this page changed a default. :func:`mhcmatch.complement.score` scores exactly as it
   did; the parameters described below are opt-in, and the reduced form is a *measurement*, not yet
   a shipped artifact.
