...
# `_generated` holds the model fragments that `.. include::` pulls into models.rst. Without it
# here Sphinx also builds each as a standalone page, linked from nowhere and in the search index;
# they dodge the orphan warning only because `include` registers them in `env.included`.
# `include` reads the file directly, so excluding them from the build is safe.
exclude_patterns = ["_build", "Thumbs.db", ".DS_Store", "_generated"]

html_theme = "pydata_sphinx_theme"
# `site-nav.html` lives here; without this the theme cannot find it and the build dies inside
# `html-page-context` with a message that names the theme's own directories and not yours.
...
html_theme_options = {
    ...
    # **The sidebar was a flat wall of full page titles**, in source order, with no grouping and
    # nothing marking where the reader is -- so it was scenery, not navigation. `index.rst` now
    # carries five captioned toctrees; these options make the theme render that structure rather
    # than flatten it.
...
# **The stock `sidebar-nav-bs` shows only the CHILDREN of the current top-level page**, and most
# pages here are top-level siblings -- so it rendered a bare "Section Navigation" heading with
# nothing under it, on almost every page. `site-nav.html` renders the whole captioned tree instead.
# The landing page is excluded: it has its own card grid and a duplicate tree beside it adds
# nothing. Same template, and the same reasoning, as `seqtree`.
html_sidebars = {
    "**": ["site-nav"],
    "index": [],
}
