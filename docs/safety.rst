...
.. note::
   ...
   ``--screen`` builds **one** whole-proteome text index, whatever register lengths are asked for,
   and every unit's registers then resolve in a single query — so screen the whole candidate list in
   one process, never one unit per invocation. Measured on the human proteome (147,506 proteins,
   69,578,135 residues) with 3,000 units, i.e. 222,000 distinct registers across all four class-I
   lengths: **0.9 s and 0.8 GB peak**, against **172.5 s and 11.1 GB** when the index was built per
   length. **Without the flag no safety check runs at all** and the cassette carries whatever it was
   handed. It used to be the one process on a cluster that needed 48 GB; it no longer is, and
   ``integrations/nextflow/mhcmatch/nextflow.config`` sizes it at 8 GB either way.
