...
nine-term ``mhc1.human.neoantigen`` fit. The other seven artifacts are the same machinery on a
different term set: six for both class-II neoantigen cells, and for the ``pathogen`` cells two at
class I and three at class II. For the eight fitted cells themselves --- their coefficients with bootstrap intervals, what each was fitted
...
:doc:`models` prints all eight cells in full --- *generated from the artifacts on every build*, which
...
parameter paid for on no information. Repaired, it fits **+0.2222** in v12. (Which term is largest moves with every refit; ``mhcmatch rank
...
``neoag``; the cassette map (``cassette build --map``) carries ``core`` unconditionally, beside the
