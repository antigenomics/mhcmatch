...
| `mhcmatch_vector_map_alleles_mhc2` | `null`, or `--alleles_mhc2` when that is set | the recipient's class-II allotypes. Without them the map is class I only and **`self_help` is not computed** — whether a unit's CD8 epitope has CD4 help from the *same* unit, which is the difference between a long peptide that raises both responses and one that needs a borrowed universal helper. A run using one literal class-II panel (`--alleles_mhc2`) gets this for free; a **per-donor** list cannot travel this way today, because it would have to be a sixth element of `MHCMATCH_CASSETTE`'s input tuple |
