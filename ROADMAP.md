...
- ~~**The MHC-II binder gate is a length detector**~~ **fixed in v0.6** — `bench/results/binder_gate_length_bias.md`. The gate was `anchor_score > 0.0` (`store.py`), but the score is a log-odds comparable across neither length nor allele, so a fixed cut was neither a binding test nor a constant error rate: a random 21-mer passed 98% of the time (pre-v0.6 `register="max"`), and for some DP alleles the *average random 19-mer* already scored above 0. **Two things did not fix it**, both recorded in that file: `register="marginal"` halves the length inflation but leaves a Jensen residual (+4.44 → +2.28 nats, 9mer→21mer), and no allele-agnostic per-length offset table can work — the across-allele spread (1.2–1.4 nats at *every* length) is larger than the within-allele length spread. The fix is `AnchorModel.null_threshold(allele, length, alpha)`: an empirical null quantile per (allele, length), lazily estimated and cached, exposed as `restriction(..., gate_alpha=0.05)`. Measured FPR is now flat in length at the nominal rate (2–8% across L=9–21 vs 3–63% before). MHC-I is fixed by the same change (its `s>0` FPR ranged 1–29%). **The score itself is still length-biased on purpose** — only the gate is fixed; `mhcmatch.ligand` must keep ranking spans by the flank model, and `%rank` remains the cross-allele-comparable *reported* number.
