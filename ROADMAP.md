...
- **The MHC-II binder gate is a length detector** (`store.py:390`, `anchor_score > 0.0`). `AnchorModel.score` rises with peptide length even on pure noise. Under the pre-v0.6 `register="max"` this was a max over more frames (a **random** 15-mer passed the gate 85% of the time, a 21-mer 98%). The v0.6 `register="marginal"` default **halves but does not remove it** — the frame count is normalized away, but a Jensen residual towards `log E[e^s]` remains (random peptides, DRB1_1501, 9mer→21mer: +4.44 nats under `max`, **+2.28** under `marginal`; gate pass-rate 26%→98% vs 26%→66%). It saturates rather than growing like `ln n`, but the gate is still not a binding test. Ranking benchmarks are unaffected (a monotone length offset cancels when candidates are length-matched), which is why it went unnoticed. **Still open**; the fix is a length-matched `%rank` (option 1 in `bench/results/binder_gate_length_bias.md`), which needs `calibrate.random_peptides(length_bg="uniform")` wired — it exists, is documented as the right null, and no production call site passes it. `mhcmatch.ligand` is unaffected — it never uses `AnchorModel.score` to rank spans.
