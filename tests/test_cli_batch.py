...
def test_rank_coefficients_dumps_the_shipped_artifact(capsys):
    ...
    assert rows[0] == ["block", "term", "coef", "sd", "boot_sd", "z", "p",
                       "z_boot", "p_boot", "ci_low", "ci_high", "sign_stab"]
    m = R.aggregate()
    ...
    # `sd`/`z`/`p` are the CR1 cluster sandwich and `boot_sd`/`z_boot`/`p_boot` the cluster
    # bootstrap, and BOTH are printed because they are different estimators of the same
    # coefficient: `expr_lvl` reads sd 0.1601 against boot_sd 0.0953 on the shipped artifact. The
    # paper's tables and their captions quote the bootstrap pair, alongside `ci95` and
    # `sign_stability`, which are bootstrap already -- so a dump carrying only the sandwich pair
    # beside a bootstrap interval is what let a figure disagree with its own legend.
    assert [float(r[7]) for r in rows[1:]] == [round(z, 2) for z in m["z_boot"]]
    assert [r[8] for r in rows[1:]] == [f"{p:.3g}" for p in m["p_boot"]]
...
def test_asking_for_a_model_that_was_never_fitted_is_an_error_not_a_traceback():
    ...
    Both halves are still asserted, because "it resolves" and "the unfitted case still refuses"
    are two different contracts and filling the last cell would otherwise have quietly deleted the
    second. With no unfitted cell left, the second half removes a key rather than finding one
    missing -- the branch has to survive a registry that is complete today and may not be
    tomorrow.
    """
    from mhcmatch import rank as R
    ...
    saved, cached = R.AGGREGATE_ARTIFACTS, R._AGG
    try:
        R.AGGREGATE_ARTIFACTS = {k: v for k, v in saved.items()
                                 if k != ("mhc2", "human", "pathogen")}
        R._AGG = {}
        with pytest.raises(ValueError, match="no fitted artifact"):
            R.aggregate("mhc2", "human", "pathogen")
    finally:
        R.AGGREGATE_ARTIFACTS, R._AGG = saved, cached
