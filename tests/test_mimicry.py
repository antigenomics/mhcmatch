...
class _EmptyIndex:
    ...
    Fakes both halves of the `seqtree.Index` search interface. `features` batches per
    (component, channel, length) and so calls `search_batch`; `search` stays because this is a fake
    of that interface rather than of one caller, and a stub that only implements what today's
    caller happens to use turns the next batching change into a mystery AttributeError."""

    def search(self, q, params):
        return []

    def search_batch(self, qs, params, threads=0):
        return [[] for _ in qs]
...
@pytest.mark.hfdata
def test_the_class_two_self_table_is_a_ligandome_and_class_one_stays_the_proteome():
    """``self`` at class II is a **ligandome**; at class I it is still the host proteome.

    The proteome saturated the class-II table -- 8,000 of 8,000 3-mer cells at ~111 M window mass --
    and a near-uniform density cannot discriminate, which is the channel's whole job. A proteome
    says what the host *encodes*; a tolerance channel is about what it *presents*, so class II reads
    IEDB class-II ligands whose source protein is the host's own (``mimics.CLS_REFS``). Being a
    ligand deposit it has a register, so it takes the same per-window class-II face ``thymus`` and
    ``viral`` do rather than the raw-k-mer path a proteome needed.

    Two things are asserted, and the second is the one that matters: **class I is bit-identical**.
    ``C_corpus_self`` is a fitted feature of two shipped class-I artifacts, so a class-II definition
    may change and a class-I table may not.
    """
    import numpy as np

    t2, n2 = mimicry.corpus_counts(None, "mhc2", "self")
    # A ligandome is ~4x smaller in mass than the proteome it is drawn from and, unlike it, is
    # allowed to leave cells empty: what does not get presented does not get counted.
    assert 1e7 < n2 < 1e8, "the class-II self table is a ligandome, not the 111 M-window proteome"
    assert (t2 > 0).sum() >= 7000, "a 421k-peptide ligandome fills nearly every 3-mer cell"
...
def test_the_vendored_corpus_tables_are_current_and_rebuild_bit_identically():
    ...
    2. **Content** -- a live rebuild of the deposit channels is bit-identical to the shipped
       table. Those are 0.7-1.6 s each; the two class-I ``self`` channels are 51.4 s and 14.5 s and
       are checked on their totals instead, which is the number a wrong face or a changed proteome
       moves first. The class-II ``self`` channels are a ligandome now, so their totals are the
       ligandome's, not the proteome's.
    ...
    # the expensive channels, on their totals -- see the docstring. `full` says whether every
    # 3-mer cell must be occupied: a PROTEOME contains every 3-mer by construction, a LIGANDOME
    # only the ones that get presented, so class-II `self` is allowed an empty cell and class-I
    # `self` is not. That difference is the whole point of the class-II reference change.
    for cls, comp, sp, want, full in (("mhc1", "self", "human", 121_968_158.0, True),
                                      ("mhc1", "self", "mouse", 112_565_681.0, True),
                                      ("mhc2", "self", "human", 29_519_304.0, False),
                                      ("mhc2", "self", "mouse", 1_914_553.0, False)):
        T, n = mimicry.corpus_counts(None, cls, comp, self_species=sp)
        assert n == want, (cls, comp, sp, n)
        assert (T > 0).all() if full else (T > 0).sum() >= 7000, (cls, comp, sp, int((T > 0).sum()))
...
def test_a_mouse_class_i_query_reads_the_human_corpus_tables_throughout():
    ...
    # silently is the failure this policy exists to make explicit.
...
def test_a_mouse_run_scores_against_the_same_tables_the_human_artifact_does():
    ...
    from mhcmatch import cli, mimicry
    ...
    def table(species, comp):
        spec = mimicry.corpus_spectrum(cls="mhc1", components=(comp,), k=g["k"],
...
def test_the_reference_cache_actually_writes(tmp_path, monkeypatch):
    """`_refs_to_disk` swallows every exception on purpose -- it is derived data, and a failed
    cache write must never fail a run. That is exactly why it needs a test that the write HAPPENED.

    It imported `proteome._unlink`, which `d929439` deleted along with the proteome disk cache. The
    `ImportError` was raised inside the function's own `except Exception` and swallowed, so from
    2026-09-20 the cache wrote nothing, `_refs_from_disk` missed every time, and the 63.9 s of
    `load_references` this exists to save was paid again by every process in a fan-out. Nothing
    failed and nothing was logged; it quietly stopped working. A swallowed exception needs a
    positive assertion that the work occurred, not a check that it did not raise."""
    import numpy as np
    ...
    class _Backing:
        _win = np.array([b"GILGFVFTL"], dtype="S9")
        _src = ["SRC"]

    refs = {("self", "tcr", 9): (Index.build(["GILGFVFTL"], alphabet="aa"), 1, _Backing())}
    mimicry._refs_to_disk("cache-probe", refs)

    d = mimicry._refs_dir("cache-probe")
    assert d is not None, "no cache dir under MHCMATCH_CALIBRATION_CACHE"
    assert "manifest.npz" in os.listdir(d), "the cache wrote nothing, and said nothing"
    assert mimicry._refs_from_disk("cache-probe"), "written, but not readable back"
