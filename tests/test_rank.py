...
def test_aggregate_carries_the_fit_provenance_a_reader_needs():
    ...
    # v12 is 339,595 / 594 against v11's 339,599 / 597, and the whole difference is one screen:
    # IEDB_neoag 424 -> 420 rows, 234 -> 231 positives, every other screen identical. v11's count
    # is not reproducible from this repo at any scorer epoch -- the corpus stage has emitted 420
    # since `99ffac4`, so v11 predates the chain that produces the frame.
    assert a["fit"]["rows"] == 339595 and a["fit"]["positives"] == 594
    ...
    assert a["version"] == 12 and "former_name" not in a
...
def test_default_score_is_the_fitted_aggregate_not_the_gate():
    ...
    want = aggregate_score({"binder": [2.0, 0.1], "log10a": [R._logit10(0.9), R._logit10(0.01)],
                            ...
                            # These rows name no gene, so the normal-tissue level is unavailable
                            # and comes back nan -- which `aggregate_score` maps to 0 on the z
                            # scale, the training mean. Built through the same helper so the
                            # expectation cannot drift from what `_finish` does.
                            "expr_norm": expr_norm_level(rows, floor),
...
def test_finish_supplies_every_feature_the_artifact_declares():
    ...
    assert R.AGGREGATE_FEATURES.count("expr") == 0
