...
class AnchorModel:
    ...
    def __init__(self, store, cls="mhc1", anchors=None, h=2.0, prior_strength=10.0, anticore=0.0,
        ...
        if length_prior and cls == "mhc1":
            ...
            # Class I only, so this reads (8,9,10,11) and the class-II widening never reaches it.
            self._len_bg = 1.0 / len(LIGAND_LENGTHS[cls])     # a screen tiles every length uniformly
        else:
            self.len_prefs = None
            self._len_bg = None
        self._len_cache = {}
    ...
    def _anchor_logodds(self, residues, allele, raw, eps, mask=None, contexts=None, length=None,
        ...
        if contexts is None:
            # Without a context the term is a pure function of (anchor index, residue) once
            # (allele, raw, eps, length, k) are fixed -- so it is a 20-entry table per anchor, and
            # recomputing it per peptide was the whole cost: building one allele's calibration
            # background ran this 144,695 times for ~100 distinct values.
            #
            # **The same floats, added in the same order.** The table stores exactly the term the
            # loop below computes and the sum stays this sequential `+=`, so no value and no
            # rounding changes -- which matters because `predict.SCORER_EPOCH` would otherwise have
            # to move and invalidate every cached background in two repos.
            rows = self._lo_table(allele, raw, eps, length if use_len else None, k)
            for i in idxs:
                r = residues[i]
                if r is None:
                    continue
                v = rows[i].get(r)
                if v is None:                    # non-canonical residue: keep the table at 20 keys
                    j = self.anchors[i]
                    th = (self._dist_len(j, allele, raw, length) if use_len
                          else self._dist(j, allele, raw, k))
                    v = math.log((th.get(r, 0.0) + eps)
                                 / (self._bg_prob(j, r, None, allele) + eps))
                s += v
            return s
        for i in idxs:
    ...
    def _lo_table(self, allele, raw, eps, length, k):
        ...
        # **A vendored model never ran `__init__`.** `load_vendored_anchor_model` rebuilds the object
        # from the shipped payload, so an attribute added to `__init__` is simply absent on every
        # model the wheel ships -- which is most of them. Initialise on first use rather than
        # assuming construction: the clear sites assign unconditionally and stay correct either way.
        cache = getattr(self, "_lo_cache", None)
        if cache is None:
        ...
        rows = []
        for j in self.anchors:
            th = (self._dist_len(j, allele, raw, length) if length is not None
                  else self._dist(j, allele, raw, k))
            rows.append({r: math.log((th.get(r, 0.0) + eps)
                                     / (self._bg_prob(j, r, None, allele) + eps)) for r in _AA20})
        cache[key] = rows
        return rows
...
_VENDORED_MODELS = {
    ...
    # 2026-09-20.** `restriction()` and `vote()` take `anchor_model`'s own defaults --
