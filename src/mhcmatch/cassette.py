    ...
    **Four optional parameters, all off by default and all bit-identical at their defaults.**
    ...
    ``selectivity``
        ...
        **Charged to the objective, never to ``p``.** ``p`` is a calibrated marginal that
        ...
        than fitted for the same reason ``gamma`` is: the shipped EPIC model fits ``expr_lvl``
        **+0.3704** and ``expr_norm`` **+0.4950**, both positive, because it was fitted on *will
        this respond* and a gene transcribed everywhere responds more often. "High in tumour, low in
