...
producer can write, since :func:`parse_variant_header` reads a self-describing ``key=value`` header.
...
* **native** (:func:`write_native`) -- one row per predicted binder with presentation **%rank**,
  **P(present)**, **band**, **IC50 (nM)**, the wild-type counterpart + **agretopicity / amplitude /
  DAI**, the **synthesise / model** peptides, and the anchor / TCR-facing decomposition. **This is
  the output to read**; everything mhcmatch computes is in it.
* **scored-csv** (:func:`write_scored_csv`) -- the same calls in a fixed 57-column wide CSV
  (:data:`SCORED_COLUMNS`). A **compatibility export**, kept so a caller whose downstream already
  reads that schema can swap its class-I/class-II binding predictor without touching anything else.
  It is not part of the generic path and it carries columns this package never fills.
...
from .store import LIGAND_LENGTHS

#: Binding-length k-mers tiled per class (pipeline ``params.mhcI_epit_len`` / ``mhcII_epit_len``).
#: **An alias, not a copy** -- see :data:`mhcmatch.store.LIGAND_LENGTHS` for why there is one ladder.
KMER_LENS = LIGAND_LENGTHS
...
#: Spellings a ``key=value`` header may use for a field this module keys under another name, so a
#: producer writes the obvious word rather than ours. Lower-cased before lookup.
_KV_ALIASES = {"gene": "gene_name", "gene_symbol": "gene_name", "symbol": "gene_name",
               "consequence": "subtype", "variant_type": "subtype", "effect": "subtype",
               "transcript": "transcript_id", "expression": "tpm", "expr": "tpm",
               "chr": "chrom", "chromosome": "chrom", "position": "pos", "start": "pos",
               "reference": "ref", "variant": "alt",
               "wt": "wt_window", "mt": "mut_window", "mut": "mut_window"}

#: The four positional header families, which a ``key=value`` header must not be mistaken for.
_POSITIONAL_KINDS = ("Somatic", "Fusion", "CNV", "Isoform")
...
def parse_variant_header(header: str) -> dict:
    ...
    **The generic form is ``key=value``**, separated by semicolons or whitespace and in any order::
        ...
        >id=S1_1;gene_name=BRAF;chrom=chr7;pos=140753336;ref=A;alt=T;
         subtype=missense_variant;tpm=47.9;wt_window=...;mut_window=...

    A producer emits only the fields it has, the reader needs no agreed field order, and adding a
    field cannot shift the meaning of the ones beside it -- which is the one thing a positional
    schema cannot promise. Unknown keys are ignored, and :data:`_KV_ALIASES` accepts the obvious
    synonyms (``gene``, ``consequence``, ``chromosome``, ``expression``).

    Four **positional** families are also read, because they are what the pipelines this module was
    first written against emit: ``Somatic:``, ``Fusion:`` and ``CNV:`` are colon-delimited and
    ``Isoform:`` is pipe-delimited after its type. All five key into the same field names, so a
    consumer reads one dict. Best-effort and never raising -- a field the header does not carry
    comes back empty.
    ...
    **The non-``Somatic`` families are parsed rather than skipped because they are the
    ...
    # Gated on the family prefix, not merely on the presence of `=`: a positional `Somatic:` window
    # whose gene name or sequence happens to contain one still parses positionally.
    if "=" in header and kind not in _POSITIONAL_KINDS:
        for tok in re.split(r"[;\s]+", header.strip()):
            k, eq, v = tok.partition("=")
            k = k.strip()
            k = _KV_ALIASES.get(k.lower(), k)
            if eq and k in var:
                var[k] = v.strip()
        # `type` is the provenance and `subtype` the consequence. A kv header naming only the
        # consequence is a somatic variant, which is what `variant_product` reads next.
        var["type"] = var["type"] or "Somatic"
        var["source"] = header
        return var
    var["type"] = kind
    if kind == "Somatic":
...
#: The modules whose **source** decides what a calibrated score is. Hashed into every cache key by
#: :func:`_scoring_digest`, so a change to any of them invalidates every cached background without
#: anyone remembering to do anything.
_SCORING_MODULES = ("predict", "calibrate", "diffusion", "affinity")


@lru_cache(maxsize=1)
def _scoring_digest() -> str:
    """A digest of the scoring code itself, so a cache entry can never outlive the code that wrote it.
    ...
    The int stays, and is still worth bumping, because it is what a **human** reads in a record and
    what the benchmark's feature frame keys its freshness guard on. This is the belt to its braces.

    Falls back to :data:`SCORER_EPOCH` alone if a source file cannot be read --- a zipimport or a
    frozen build --- because refusing to score would be a worse answer than a coarser key.
    """
    h = hashlib.sha256()
    try:
        for name in _SCORING_MODULES:
            src = inspect.getsource(importlib.import_module(f".{name}", __package__))
            h.update(src.encode())
            h.update(b"\x00")
    except (OSError, TypeError):
        return f"e{SCORER_EPOCH}"
...
def _fingerprint(store, cls, background, footprint, head):
    ...
    artifacts are vendored and are rebuilt on version bumps -- :data:`SCORER_EPOCH`, and
    :func:`_scoring_digest`, which covers the one thing the rest cannot and the epoch only covers
    when a human remembers: a change to the scoring code inside one released version.
    """
    from . import __version__
    panel = store._panel[cls]
    return "|".join([__version__, str(SCORER_EPOCH), _scoring_digest(), cls, background, footprint,
                     head, str(len(panel.epitopes)), str(len(set(panel.alleles)))])
