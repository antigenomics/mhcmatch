...
__all__ = [
    ...
    "chebyshev_score", "corner", "survival", "p_at_least", "visibility", "n_effective", "coverage",
...
def visibility(p, block=None) -> dict:
    r"""``V = 1 - prod_i (1 - p_i)`` over a set nobody chose, and what losing one allotype costs.

    **The observational read, and it carries no design parameter.** :func:`survival` and
    :func:`p_at_least` answer this question for a cassette some rule *selected*, under a block
    model with a live probability ``q``. This answers it for the set a tumour happens to present:
    every unit, weight one, no ``q``, no coupling, no ``gamma`` and no ``rho``. A tumour is under
    selection too, but towards escape, and that dynamics is not modelled here --- so there is no
    designer's risk to price and nothing to maximise over. Keeping the two apart is the point:
    see :func:`mhcmatch.cassette.goal_energy` for the design side, which is where those parameters
    belong.

    ``V`` is the probability that at least one presented unit elicits a response, so it
    **saturates**: once a few strong antigens are present, further units move it very little. That
    is what makes it a whole-tumour read rather than a count, and why it decouples from mutational
    burden where a sum does not.

    **HLA loss of heterozygosity, in the same units.** Pass ``block`` --- one allotype label per
    unit --- and losing allotype ``a`` deletes its terms. With ``S = sum_i log(1 - p_i)`` and
    ``S_a`` that same sum over the allotype's own units, visibility after the loss is
    ``1 - exp(S - S_a)``; the worst single loss is the allotype minimising it, which is the one
    whose ``S_a`` is most negative. ``loh_cost`` is how much visibility the tumour's most
    protective allotype carries --- the McGranahan escape route, priced on the same scale as ``V``.

    Returns ``visibility``, ``log1m`` and ``n_units``, and with ``block`` also
    ``visibility_loh``, ``loh_cost``, ``lost_allotype`` and ``n_allotypes``. ``log1m`` is
    ``S`` in nats --- negative, linear in the units and unbounded below, so it separates the
    saturated top of a cohort that ``visibility`` itself cannot.

    **Why this is a closed form and not a call into** :func:`p_at_least`. At ``q = 1`` that function
    returns exactly this number whatever the blocking, so the design record asked for the call
    rather than a second spelling of one quantity --- normally the right instinct, and
    this repository names that hazard in five other places. It does not hold here:
    :func:`survival` convolves one Poisson binomial per block, which is **quadratic** in the unit
    count, and ``V`` is defined over a *whole presented set* --- hundreds to thousands of units, on
    every donor of a cohort. The closed form is linear. The two spellings are held together by a
    test that asserts they agree rather than by one calling the other
    (``test_visibility_agrees_with_p_at_least_where_the_two_overlap``).
    ...
    (0.5, 'A')
    """
    p = np.clip(np.asarray(p, dtype=float), 1e-12, 1.0 - 1e-9)
    lq = np.log1p(-p)
    tot = float(lq.sum())
    # `log1m` is `S` itself, and it is returned rather than discarded because `V` SATURATES:
    # measured over 7,261 tumours it runs median 0.7976, q95 0.99999929 and q99 = max =
    # 1.00000000, so at six decimals the top percentile of a cohort prints as one
    # indistinguishable block of `1.000000`. The information is entirely in the complement.
    # `S` is linear in the units, unbounded below, and never saturates -- it is what a donor
    # ranking should read.
    out = {"visibility": float(-np.expm1(tot)), "log1m": tot, "n_units": int(p.size)}
    if block is None:
        return out
    lab = np.asarray([str(x) for x in block])
    if lab.size != p.size:
        raise ValueError(f"visibility: {p.size} probabilities against {lab.size} block label(s)")
    keys = np.unique(lab).tolist()
    # The most protective allotype is the one whose own sum of log(1 - p) is most negative:
    # dropping it leaves the largest prod(1 - p), hence the smallest visibility.
    per = {k: float(lq[lab == k].sum()) for k in keys}
    worst = min(per, key=per.get)
    out["visibility_loh"] = float(-np.expm1(tot - per[worst]))
    out["loh_cost"] = out["visibility"] - out["visibility_loh"]
    out["lost_allotype"] = worst
    out["n_allotypes"] = len(keys)
    return out


#: The ordered header ``mhcmatch visibility`` emits, exported for the same reason
#: :data:`mhcmatch.cassette.SELECT_COLUMNS` is: a workflow stub has to type this header and must not
#: type it by hand. Two stubs had already drifted before those tuples existed.
#:
#: ``field`` names which field ``foot_lam`` was read on and is **always** ``score``, the raw
#: aggregate log-odds. That is not a placeholder for a future option: the design's ``lam`` is read on
#: the objective field ``p - (gamma/2) p (1 - p)`` inside ``cassette score``, and offering that
#: reading here would put ``gamma`` back into an arm that has none. The column exists so a reader of
#: the table never has to guess which of the two ``lam`` scales it is holding.
VISIBILITY_COLUMNS: tuple = (
    "donor", "pool_n", "k", "offset", "field", "vis_escape", "log1m_vis",
    "vis_escape_drv", "vis_escape_pas", "at_least_k", "vis_at_least", "vis_loh",
    "loh_cost", "lost_allotype", "n_allotypes", "foot_lam", "lam_degenerate", "foot_energy",
    "foot_gap",
)
...
def betabinom_rho(m, k, profile: bool = True) -> dict:
    ...
    try:
        ...
        raise ImportError("betabinom_rho needs SciPy: pip install 'mhcmatch[stats]'") from exc
    m = np.asarray(m, dtype=float)
    k = np.asarray(k, dtype=float)
