...
_SCOPES = (0, 1, 2, 3)
#: Distinguishes "the caller passed no tally" from "the tally is legitimately ``None``" (no
#: neighbours at any scope), which is a meaningful result and not a missing argument.
_UNSET = object()
#: **The ligand lengths tiled per class, and the ONE definition of them.** `predict.KMER_LENS`
#: is this object, not a copy: the package carried four ladders that agreed on class I (8-11) and
#: disagreed on class II in every one -- `(15,)` on the rank path, 13-18 here, 12-15/12-20 in
#: `vector`, 11-25 in `mimics` -- which is how "a class-II ligand runs to 21" and "we tile 15-mers
#: only" were both true of one library. `mimics.CANONICAL_LEN` stays separate on purpose: it sizes
#: proteome *indexes*, where a width costs disk rather than accuracy.
#:
#: **Class I runs to 11 and class II to 21, which is the author's specification and not a fit to
#: the panel.** The panel agrees with it: class-II epitopes run 9-23+ with only 24.5 % at length 15
#: (84,405 of 343,956), and 12-21 covers 93.5 % of that mix (321,550) against 12-20's 92.0 %. An
#: earlier revision of this constant stopped at 20 because a benchmark ladder
#: (`vector.MHC2_MAP_LENGTHS`) did; that was a ladder chosen for a different job, and the stated
#: maximum is 21. The %rank null was always drawn from the full mix, so the single-width tiler was
#: the thing out of step with it, not the wide one. The cost is nothing: 291 -> 2,900 windows on a
#: 305-aa record against a 209 s per-run scorer build.
LIGAND_LENGTHS = {"mhc1": (8, 9, 10, 11), "mhc2": tuple(range(12, 22))}
...
class _Panel:
    ...
    def tally_many(self, queries, lo=10, hi=100, threads=0):
        ...
        one-element list, four times per peptide, with threads pinned to 1. Over a protein scan that
        is ``4 x windows`` calls where four suffice.

        **The scope widening stays per query, which is the whole subtlety.** Each query widens only
        until *it* has ``lo`` candidates, so a later scope must re-ask only the queries still short
        ...
        cands: list[list] = [[] for _ in queries]
        pending = list(range(len(queries)))
        for sc in _SCOPES:
            if not pending:
                break
            p = SearchParams(max_subs=sc, engine="seqtm")
            got = self.index.seed_and_gather([feats[i] for i in pending], p, 1, -1, threads)
            still = []
            for i, res in zip(pending, got):
                cands[i] = [c for c in res if self.epitopes[c.peptide_id] != queries[i]]
                if len(cands[i]) < lo:
                    still.append(i)
            pending = still
        return [Counter(self.alleles[c.peptide_id] for c in cs[:hi]) if cs else None
                for cs in cands]
...
class Store:
    ...
    def _panel_spelling(self, panel):
        ...
        **The panel and the pseudosequence tables do not spell an allele the same way**, and until
        2026-09-20 this method required the panel's. The panel writes ``HLA-A*02:01`` and ``H-2Kd``;
    ...
    def panel_alleles(self, cls, alleles="all", missing: list | None = None):
        """Public :meth:`_allele_set`: the queried names in the panel's spelling, ``"all"`` for every one.

        A caller outside this class must not reach into ``self._panel`` to do this -- ``_allele_set``
        is the one place that folds both of the package's allele vocabularies together and the one
        place that reports what it could not match.
        """
        return self._allele_set(self._panel[cls], alleles, missing=missing)
    ...
    def restriction(self, peptide, cls=None, alleles="all", top=10, alpha=0.05, diffuse=False,
                    calibrated=False, _tally=_UNSET):
        ...
        **gates**: an allele is a binder if it is vote-significant *or* the anchors are plausible.
        ...
        panel = self._panel[cls]
        # `_tally` lets a batch caller (`scan_protein`) supply the vote tally it already computed
        # for every window in one `tally_many` call. Private and positional-free on purpose: it is
        # a performance seam, not an interface -- pass nothing and the behaviour is unchanged.
        tally = panel.tally(peptide) if _tally is _UNSET else _tally
        ...
        if diffuse:
            from .calibrate import WEAK_RANK
            # band_for, not calibrate.band: `band` defaults to the class-I pair
            # (0.5 / 2.0), so a class-II ligand at %rank 5.0 -- a textbook weak binder --
            # came back `non-binder` here while `predict` called the same number `weak`.
            # One library, one label. Lazy like the _fingerprint import above, because
            # predict imports store at module level.
            from .predict import band_for
        out = []
        for a in allele_set:
    ...
    def scan_protein(self, protein, cls="mhc1", alleles="all", lengths=None, alpha=0.05, top=3,
        ...
        lengths = lengths or LIGAND_LENGTHS[cls]
        ...
        panel = self._panel[cls]
        for L in lengths:
            # One `tally_many` for every window of this length, rather than four single-query
            # `seed_and_gather` calls per window. A 300 aa protein at one length is ~292 windows:
            # 1,168 C++ entries become at most 4.
            wins = [(i, protein[i:i + L]) for i in range(len(protein) - L + 1)]
