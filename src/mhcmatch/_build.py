...
TARGETS = {
    ...
    # Until 2026-09-20 these were shipped but *unchecked*: `--check` covered 11 of the 27 files
    ...
    "aggregate": ("the EPIC neoantigen and pathogen scorers", None,
                  ["aggregate_mhc1.json", "aggregate_mhc1_mouse.json",
                   "aggregate_mhc2_mouse.json", "aggregate_mhc2_human.json",
                   # The nine-term second fit of the class-II neoantigen cell, reached only
                   # through `rank.AGGREGATE_VARIANTS`. Listed here so `--check` refuses a release
                   # that drops it; it is not a default and does not appear in
                   # `rank.AGGREGATE_ARTIFACTS`.
                   "aggregate_mhc2_human_corpus.json",
                   "aggregate_mhc1_pathogen.json",
                   "aggregate_mhc1_mouse_pathogen.json",
                   "aggregate_mhc2_human_pathogen.json",
                   "aggregate_mhc2_mouse_pathogen.json"]),
...
EXTERNAL = {
    ...
    "aggregate": ("python bench/epic/fit.py --physchem rose_af5 --presentation binder --density log10a"
                  ...
                  "                     # human class I, PATHOGEN mode (Kesmir corpus)\n"
                  "    python bench/pathogen/fit_tcell.py --cls mhc1 --species mouse"
                  "        # and the three other\n"
                  "    python bench/pathogen/fit_tcell.py --cls mhc2 --species human"
                  "        # (cls, species) cells,\n"
                  "    python bench/pathogen/fit_tcell.py --cls mhc2 --species mouse"
                  "        # on the IEDB T-cell corpora"),
