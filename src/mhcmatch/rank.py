...
__all__ = ["GATE", "Ranked", "rank_fasta", "rank_table", "gate_probability",
           ...
           "AGGREGATE_VARIANTS",
           "TERMS_MOUSE_EXPECTED", "TERMS_MHC2_EXPECTED", "TERMS_PATHOGEN_EXPECTED",
...
#: a 57-column table until 2026-09-20.
...
#: How an INPUT table may spell the columns this module reads, best first. **This module holds the
#: one definition**; :mod:`mhcmatch.cli` imports these names rather than repeating them.
#:
#: They were typed out in both places until 2026-09-20, on the rationale that ``cli`` could not
#: import ``rank`` at module level without pulling numpy into every ``mhcmatch --help``. That was
#: not true: ``cli`` already imports ``POOL_PREVALENCE`` from here, this module is stdlib-only at
#: import time, and numpy arrives through ``__init__`` -> ``cassette`` regardless. What the
#: duplication did cost was real -- a pipeline candidate table spelling the peptide ``epitope`` was
#: accepted by ``rank`` and refused by ``neoag``, ``mimicry`` and ``cassette select`` in one chain.
#: One import cannot drift; two copies and a guard test only report that they have.
#:
#: The upstream spellings are pVACseq's: ``MT Epitope Seq``, ``HLA Allele``, ``Gene Name``,
#: ``WT Epitope Seq``, ``Gene Expression``. Resolving them here is what lets a standard
#: neoantigen table be a native input rather than something a caller renames first.
PEPTIDE_COLUMNS: tuple = ("peptide", "epitope", "MT Epitope Seq")
ALLELE_COLUMNS: tuple = ("allele", "best_allele", "HLA Allele")
GENE_COLUMNS: tuple = ("gene", "gene_name", "gene_symbol", "Gene Name")
WT_PEPTIDE_COLUMNS: tuple = ("wt_peptide", "WT Epitope Seq")
TPM_COLUMNS: tuple = ("tpm", "Gene Expression", "Transcript Expression")


def _first(rec, names, default=""):
    """The first of ``names`` the record carries non-blank. Ordered, first present wins."""
    return next((str(rec[n]).strip() for n in names if str(rec.get(n) or "").strip()), default)
...
def columns(extended: bool = False, annotate: bool = False, score: str = "aggregate",
    ...
    **``(cls, species, mode)`` matter too, and this is the one implementation.** A run
    ...
    if score in ("aggregate", "features"):
        ...
        keep = set(stand_in(mode, cls)["features"]) if score == "features" \
...
#:
#: Nine names and, **from mouse model version 5, nine free parameters**. Versions 2-4 spent seven:
#: the last three were one fitted scalar on the human artifact's corpus direction rather than three
#: independent coefficients, and a projection with one free scalar *is* three coefficients
#: constrained to be proportional. v5 retired that axis -- `C_corpus_thymus` carries +0.2919 at sign
#: stability 0.94 on its own. The file lists all nine so one library scores both artifacts with no
#: branch.
...
AGGREGATE_ARTIFACTS: dict = {
    ...
    # 2026-09-20 refit moved every cell onto one corpus and one balanced design. That is worth
    # knowing when reading an older number: `binder` carries presentation on the T-cell corpus and
    # does not on Kesmir, and the same cell reads +0.3882 here against +0.1454 there. **Read
    # `fit.deposit` from the artifact, never this comment** -- this sentence was wrong for a day
    # because a refit landed and the comment did not, which is the whole reason the field exists.
    ("mhc1", "mouse", "pathogen"): "aggregate_mhc1_mouse_pathogen.json",
    ("mhc2", "human", "pathogen"): "aggregate_mhc2_human_pathogen.json",
    ("mhc2", "mouse", "pathogen"): "aggregate_mhc2_mouse_pathogen.json",
    ...
    # pins that, and it is the reason the table is filled in the commit that vendors the files.
}

#: **A second fit of a cell that already has one, reachable only by asking for it by name.** A
#: variant is not a mode and not a cell: ``(cls, species, mode)`` still resolves to exactly one
#: default through :data:`AGGREGATE_ARTIFACTS`, and nothing here changes what ``mhcmatch rank``
#: scores. Kept in its own table for that reason -- a key added to the registry above would move a
#: default, and `_modeldoc.ORDER` and four tests pin that registry precisely so a default cannot
#: move by accident.
#:
#: ``("mhc2", "human", "neoantigen", "corpus")`` is the nine-term fit of the class-II neoantigen
#: cell, carrying the corpus block the six-term default drops. It ships because the block became
#: *computable* in 1.20.0 -- the HLA Ligand Atlas gives 132,818 class-II peptides over 29 benign
#: tissues, where the class-II thymic fraction alone gave 27,987 -- and it is **not** the default
#: because computing it answers the question in the negative. On the 1,112 CD4+ rows (656
#: immunogenic, 157 references) every one of the three coefficients spans zero:
#: ``C_corpus_thymus`` +0.0170 (95 % CI -0.4944 to +0.3995, sign held in 53 % of resamples),
#: ``C_corpus_self`` +0.5227 (-0.3044 to +0.9815), ``C_corpus_viral`` -0.3612 (-0.8419 to +0.2928).
#: **At class II the corpus block adds nothing**, and the six-term default is the honest release.
#: Use this one to reproduce that measurement, not to score with.
AGGREGATE_VARIANTS: dict = {
    ("mhc2", "human", "neoantigen", "corpus"): "aggregate_mhc2_human_corpus.json",
}

#: **The class-II NEOANTIGEN artifacts carry six terms, not nine, and that is a specification.** A
#: corpus channel is a density over a *reference* set of peptides -- displayed self, encoded self,
#: viral -- and the two cells reach that tuple for different reasons, which is worth keeping apart:
#:
#: * **human**, since 1.20.0, because the block was computed and answers in the negative. The HLA
#:   Ligand Atlas supplies class-II references, the nine-term fit ships as a *variant*
#:   (:data:`AGGREGATE_VARIANTS`), and on its 1,112 CD4+ rows all three coefficients span zero.
#:   The six-term default is a measurement, not a gap.
#: * **mouse**, because the reference tables are still class-I geometry, so the block would be
#:   fitted against a mismatched register and is dropped outright.
#:
#: Either way the design has no `C_corpus_*` column and `aggregate_score` reads only the names the
#: artifact lists, so one library still scores all eight cells with no branch. The nine names stay
#: registered in :data:`AGGREGATE_FEATURES` because a recorded result cites the model version that
#: produced it, and a registry that drops a name cannot say what those numbers were.
#:
...
#: **The shipped pathogen artifacts carry two terms on class I and three on class II**, against the
#: nine of the neoantigen design. Each omission has its own reason and only some are weak
#: coefficients; the paragraphs below give them in the order they were established.
#:
...
#: compare it against; ``expr_lvl`` would be a number for a quantity that does not exist. Nothing
#: is imputed, because there is nothing to impute *from*.
#:
#: ``C_corpus_viral`` is dropped **by this fit's deposit, not by the mode**, and **not** because
#: the reference is impure. A corpus channel is defined by the *nature* of its compartment --
#: ``viral`` is "foreign in origin, observed presented on MHC" -- and every peptide in that deposit
...
#: transfer to nothing.
#:
#: A ``pathogen`` fit whose negatives do *not* overlap the table keeps the channel and carries one
#: term more. That is why nothing in this module selects channels by mode: the admissible set is
#: read off the artifact's own ``features`` list, per :func:`stand_in` and
...
#: measured it.
#:
#: **Keyed by class, because the two classes do not have the same admissible terms.** Class I keeps
#: one corpus channel and drops the physchem pair; class II keeps the physchem pair and drops the
#: corpus block entirely. That is not symmetry for its own sake -- it is what the fits measured on
#: the balanced corpora (2026-09-20):
#:
#: * class I: ``C_corpus_self`` is the term carrying the non-presentation signal
#:   (mouse -0.1487, p = 1.6e-08, sign stability 1.000 on 10,398 peptides), and adding Rose burial
#:   and AF5 on top moves ROC-AUC by +0.0008 (human) and +0.0003 (mouse). They did not earn a
#:   parameter.
#: * class II: there is no comparable corpus channel to keep -- the host tables are class-I
#:   geometry -- so the physchem pair is what remains beside presentation.
#:
#: A consumer must still read the artifact's own ``features`` list rather than this table
#: (:func:`stand_in`, :func:`aggregate_features`); this is the *specification* a shipped fit is
#: checked against, not the thing that selects channels at run time.
TERMS_PATHOGEN_EXPECTED: dict = {
    "mhc1": ("binder", "C_corpus_self"),
    "mhc2": ("binder", "C_phys_buried", "C_phys_charge"),
}
...
#: two models, never one model with an extra covariate. They also do not admit the same *terms*:
#: ``pathogen`` drops the expression block, which is undefined for a peptide the host does not
#: transcribe. What it does with the corpus block is **not** a property of the mode but of the
#: deposit a given fit was trained on -- see :data:`TERMS_PATHOGEN_EXPECTED` -- so two ``pathogen``
#: artifacts may legitimately carry different corpus channels, and every consumer here reads the
#: artifact's own ``features`` list rather than a table keyed on the mode.
#:
#: Every ``(cls, species)`` cell not listed in :data:`AGGREGATE_ARTIFACTS` refuses by name rather
#: than silently serving a neighbour's coefficients.
...
#: The same stand-in for a ``pathogen`` fit: **the columns that mode can be fitted on, and no
#: others.** Not ``FEATURES_ONLY`` minus a few names as a convenience -- the three it drops are
#: dropped for the reasons in :data:`TERMS_PATHOGEN_EXPECTED`, and asking for them here is an error
#: rather than a wider net. So ``mhcmatch rank pairs --epitope pathogen --score features`` computes
#: exactly what the pathogen fit fits, needs no ``--expr-floor``, and builds the ``thymus`` and
#: ``self`` corpus tables without touching the ``viral`` one.
#: Keyed by class, because the two classes admit different host corpus channels.
#:
#: **Class II carried none until 2026-09-20, and the reason given was wrong about the tables.**
#: This said "there is no class-II counterpart -- the mimicry tables are a k-mer lookup over a
#: class-I TCR face", so listing them would name always-NaN columns. But ``corpus_counts``
#: resolves a per-row class-II register on the reference side and has done since the class-II
#: cells were added: ``corpus_tables.npz`` ships ``mhc2|thymus|human|3``, ``mhc2|self|human|3``
#: and their mouse twins, all built over class-II registers. The columns were computable all
#: along and this list is what declined to compute them.
#:
#: ``thymus`` at human class II now reads the pooled HLA Ligand Atlas class-II corpus -- 132,818
#: peptides over 29 benign tissues, see :data:`mimics.CLS_REFS` -- so the pair is displayed self
#: against bulk displayed self, which is the contrast the channel is read on.
FEATURES_ONLY_PATHOGEN: dict = {
    "mhc1": {"model": "", "version": None,
             "features": ["C_phys_buried", "C_phys_charge",
                          "C_corpus_thymus", "C_corpus_self"]},
    "mhc2": {"model": "", "version": None,
             "features": ["C_phys_buried", "C_phys_charge",
                          "C_corpus_thymus", "C_corpus_self"]},
}
...
def stand_in(mode: str = "neoantigen", cls: str = "mhc1") -> dict:
    """The ``--score features`` pseudo-artifact for ``(mode, cls)``: what to compute when nothing
    scores.

    ``--score features`` has no fitted artifact to read a ``features`` list off, so it needs one
    supplied. This is the single place that answers it, read by both ``cli._aggregate_channels``
    (which corpus tables to build) and the banner ``rank`` prints (which columns it filled) -- two
    ...
    **This is the ADMISSIBLE set, not the shipped one.** A pathogen fit may be fitted on any subset
    ...
    if mode not in AGGREGATE_MODES:
        raise ValueError(f"unknown mode {mode!r}; expected one of {', '.join(AGGREGATE_MODES)}")
    if mode != "pathogen":
        return FEATURES_ONLY
    if cls not in FEATURES_ONLY_PATHOGEN:
        raise ValueError(f"unknown cls {cls!r}; expected one of "
                         f"{', '.join(FEATURES_ONLY_PATHOGEN)}")
    return FEATURES_ONLY_PATHOGEN[cls]
...
def aggregate(cls: str = "mhc1", species: str = "human",
              mode: str = "neoantigen", variant: str = "") -> dict:
    ...
    **There is no intercept and that is deliberate.** Each screen was given its own, unpenalised,
    ...
    if variant:
        key = (cls, species, mode, variant)
        name = AGGREGATE_VARIANTS.get(key)
        if name is None:
            raise ValueError(
                f"aggregate: no {variant!r} variant of cls={cls!r}, species={species!r}, "
                f"mode={mode!r}. This library ships {sorted(AGGREGATE_VARIANTS)}. Omit `variant` "
                "for the default fit of that cell.")
    else:
        key = (cls, species, mode)
        name = AGGREGATE_ARTIFACTS.get(key)
        if name is None:
            raise ValueError(
                f"aggregate: no fitted artifact for cls={cls!r}, species={species!r}, "
                f"mode={mode!r}. This library ships {sorted(AGGREGATE_ARTIFACTS)}. Scoring with "
                "another combination's coefficients would report a number computed from the "
                "wrong fit.")
    if key not in _AGG:
...
def aggregate_score(features, imputed_out: list | None = None, cls: str = "mhc1",
                   species: str = "human",
                    mode: str = "neoantigen") -> "np.ndarray":
...
def _expression_for(gene: str, observed, tissue: str | None, tumor: str | None,
                    peptide: str = "", species: str = "human",
                    mode: str = "neoantigen") -> tuple[float, bool]:
    ...
    **The tumour rung is keyed by peptide in human and by gene in mouse.** TCGA has per-peptide
    ...
    if mode == "pathogen":
        # **Undefined, not missing, and the difference decides what to return.** A pathogen epitope
        # comes from an organism the host does not transcribe, so there is no source gene to
        # measure and no matched normal to compare against. Every rung below is a substitution for
        # a value that exists and was not supplied; here there is nothing to substitute *for*. So
        # the column is NaN with `imputed=False` -- imputed would claim a rung was walked -- and
        # the pathogen artifact declares no expression term, so nothing reads it. Returning early
        # also keeps a pathogen run from importing and staging the expression references at all.
...
#: What "which groove was ``binder`` measured at" looks like when nothing says otherwise: the row's
#: own recorded allele, best presenter among the ones it names. Every run and every artifact that
#: predates :func:`_check_allele_policy` is this by construction.
RECORDED_POLICY = {"kind": "recorded", "select": "presentation"}


def _check_allele_policy(artifact: dict, run: dict | None) -> None:
    """Refuse to score rows measured under one allele policy with coefficients fitted under another.

    **An artifact with no ``allele_policy`` key is UNCHECKED**, and that is deliberate on both
    counts: every shipped fit predates the key, so nothing existing moves by one bit, and you cannot
    retroactively assert what an old fit was fitted under. Only a fit that declares a policy
    constrains the runs that use it.

    The failure this closes has no other tell. A ``binder`` minimised over a 203-allele panel is
    systematically lower %rank than one taken at the recorded groove, so a coefficient fitted on the
    first meets a shifted column when handed the second -- every score moves, with no NaN, no error
    and an unchanged header. ``n`` differing alone is reported and not refused: a donor's own three
    to six allotypes is the right panel for that donor.
    """
    want = (artifact or {}).get("allele_policy")
    if not want:
        return
    have = run or RECORDED_POLICY
    for k, flag in (("kind", "--allele-panel"), ("select", "--best-by")):
        if want.get(k) != have.get(k, RECORDED_POLICY.get(k)):
            raise ValueError(
                f"allele policy mismatch: this fit was made with {k}={want.get(k)!r} and the run "
                f"is scoring with {k}={have.get(k, RECORDED_POLICY.get(k))!r}. The coefficients "
                f"would meet a shifted column and every score would move silently. Pass {flag} to "
                f"match, or score with an artifact fitted the way you are scoring.")
    if want.get("n") and have.get("n") and want["n"] != have["n"]:
        import sys
        print(f"# allele policy: fitted over {want['n']} allotype(s), scoring over {have['n']}. "
              f"Same kind of measurement, different breadth -- comparable within a run, not across.",
              file=sys.stderr)
...
def _finish(rows: list, gate: dict | None, score: str = "aggregate",
            ...
            cls: str = "mhc1", species: str = "human",
            mode: str = "neoantigen", allele_policy: dict | None = None) -> list:
    ...
    """
    if score in ("aggregate", "features"):
        a = stand_in(mode, cls) if score == "features" else aggregate(cls, species, mode)
        if score != "features":
            _check_allele_policy(a, allele_policy)
        ...
        # **Known and deliberately unfixed (2026-09-20): `cls` is not passed to `CM.burial`.**
        # `burial` defaults to `cls="mhc1"`, so on a class-II run both C_phys columns average over
        # the class-I anchor mask -- on a 15-mer, positions [0, 1, 2, 13, 14] rather than the
        # register-aware (4, 7, 9, 12). Measured on one 15-mer the two differ by 0.0091 (0.7400
        # against 0.7309).
        #
        # It is MIS-NAMED rather than wrong. `bench/mhc2_human/fit_human_mhc2.py` computes its
        # design matrix through this same path, so the fitted coefficient and the scored column
        # are the same quantity and the class-II model is self-consistent; what is inaccurate is
        # calling that quantity a class-II burial. Fixing it moves every published class-II
        # chemistry number, so it is the author's call and not a drive-by: it is recorded in
        # ROADMAP.md rather than repaired here. Do not "fix" this without refitting both class-II
        # cells and re-reading the manuscript numbers that quote them.
        if rows:
...
def rank_fasta(store, fasta_path: str, alleles, cls: str = "mhc1", *, tissue: str | None = None,
               ...
               expr_prefilter: float = 0.0, mode: str = "neoantigen", **kw) -> list[Ranked]:
    ...
    for p in preds:
        ...
        try:
            ...
            tpm = None
        expr, imputed = _expression_for(gene, tpm, tissue, tumor, p.peptide, species, mode)
    ...
    # This path has always been best-of-panel by presentation -- `_predict_windows` scores every
    # `alleles` name at every tile and keeps the lowest presentation %rank -- so it DECLARES that
    # policy rather than defaulting to `recorded`, or a panel-fitted artifact would refuse the one
    # shape that already matched it.
    return _finish(rows, gate, score, prevalence, tumor=tumor, tissue=tissue,
                   cls=cls, species=species, mode=mode,
                   expr_floor=expr_floor, expr_prefilter=expr_prefilter,
                   keep=P.as_keep(keep),
                   allele_policy={"kind": "panel", "select": "presentation",
                                  "source": "list", "n": len(list(alleles))})


def _wins(a: "Ranked", b: "Ranked", select: str = "presentation") -> bool:
    """Does ``a`` beat ``b`` on ``select``? Both columns are ``-log10(%rank)``, so higher is better.

    **"Best" is two questions and this package ships both answers**, which is why the criterion is a
    parameter rather than a constant: presentation %rank alone is what :func:`mhcmatch.predict.
    _predict_windows` selects on, and the combined binder rank is what
    :func:`mhcmatch.predict.binder_score` sorts by. Hard-wiring either would make ``allele_scored``
    mean one thing on one code path and another on the next, with nothing on the row saying which.
    """
    pa = a.presentation if select == "presentation" else a.binder
    pb = b.presentation if select == "presentation" else b.binder
...
def _unscored(r: dict, cls: str, tissue, tumor, refs, binding_core, phys: dict,
              species: str = "human", mode: str = "neoantigen") -> "Ranked":
    ...
    try:
        ...
        tpm = None
    expr, imputed = _expression_for(gene, tpm, tissue, tumor, r["peptide"], species, mode)
    nan = float("nan")
    from . import predict as P
...
def wt_from_windows(rows, fasta_path: str) -> int:
    ...
    n = 0
    for r in rows:
        # The shared lists, not two local literals: a caller invoking this directly with a
        # pVACseq table got `pep = ""` for every row and recovered zero wild types, silently,
        # because `MT Epitope Seq` was invisible here while `_read_table` back-filled it on the
        # CLI path only. A table that already carries `WT Epitope Seq` is likewise already done.
        if _first(r, WT_PEPTIDE_COLUMNS):
            continue
        pep = _first(r, PEPTIDE_COLUMNS).strip().upper()
        if not pep:
            continue
        for mt, wt in pairs:
            i = mt.find(pep)
...
def rank_pairs(store, rows, cls: str = "mhc1", *, keep=None, tissue: str | None = None,
               ...
               expr_prefilter: float = 0.0,
               mode: str = "neoantigen",
               alleles: list[str] | None = None,
               best_by: str = "presentation",
               allele_policy: dict | None = None) -> list[Ranked]:
    ...
    **The reported ``allele`` is the one scored against, which is not always the one supplied.** A
    ...
    ``alleles`` **replaces** the row's own cell with one candidate set for every row -- the input's
    restriction is emitted verbatim in ``allele`` and is not read for scoring, and ``allele_scored``
    carries the groove that won. That is a different measurement, not a wider one: a ``binder``
    minimised over 203 grooves sits systematically lower than one taken at the recorded groove, so a
    model fitted under one must not score under the other. ``allele_policy`` is what says which,
    and :func:`_finish` refuses the mismatch. ``best_by`` picks the criterion -- see :func:`_wins`.
    """
    from . import predict as P
    from .store import binding_core

    # An empty override must never mean "score nothing" -- that is the `_allele_set` silent-drop
    # failure with a different name, and it exits 0 with a header.
    if alleles is not None and not alleles:
        raise ValueError("alleles= was given as an empty set: no groove would be scored and the "
                         "run would emit an unscored row for every candidate. Pass None to use "
                         "each row's own allele.")
    cand = list(dict.fromkeys(alleles)) if alleles is not None else None

    recs = [dict(r) for r in rows]
    for i, r in enumerate(recs):
        r["_i"] = i
        r["_row"] = {k: v for k, v in r.items() if not k.startswith("_")}
        # Every upstream spelling, accepted as an alias so a standard candidate table is a
        # native input rather than something a caller renames first. One-directional: an explicit
        # `peptide`/`allele`/`gene` always wins, and a blank cell never displaces a populated one.
        for want, names in (("peptide", PEPTIDE_COLUMNS), ("allele", ALLELE_COLUMNS),
                            ("gene", GENE_COLUMNS), ("wt_peptide", WT_PEPTIDE_COLUMNS),
                            ("tpm", TPM_COLUMNS)):
            got = r.get(want)
            # **`str(got or "")` counted a numeric `0.0` as blank**, so a library caller
            # handing in `tpm=0.0` -- a gene MEASURED as not expressed -- had it overwritten
            # by an alias and then imputed to a tissue mean. That is the largest fitted
            # coefficient, replaced on exactly the rows `expr_prefilter` exists to act on.
            # `tpm` is the one numeric field in this tuple; a falsy string is blank anyway.
            if got is None or (isinstance(got, str) and not got.strip()):
                r[want] = _first(r, names[1:])
        ...
        r["_alleles"] = cand if cand is not None else split_alleles(r["allele"], cls)
    # With the recorded allele stripped, a blank or unresolvable cell is no longer a reason to hold
    # a row out of scoring -- that is the point of the override.
    recs = [r for r in recs
            if r["peptide"] and r["peptide"].isalpha() and (cand is not None or r["allele"])]
    ...
    for allele, group in by_allele.items():
        ...
        for k, r in enumerate(group):
            ...
            rk.row = r["_row"]
            prev = out.get(r["_i"])
            if prev is None or _wins(rk, prev, best_by):
                out[r["_i"]] = rk
    ...
    return _finish(rows_out, gate, score, prevalence, tumor=tumor, tissue=tissue,
                   cls=cls, species=species, mode=mode,
                   expr_floor=expr_floor, expr_prefilter=expr_prefilter,
                   keep=P.as_keep(keep), allele_policy=allele_policy)
...
def rank_table(path: str, *, channels=None, keep=None,
    ...
    Reads a wide candidate CSV, resolving each column by name from :data:`PEPTIDE_COLUMNS` and
    its siblings (so ``epitope`` / ``MT Epitope Seq``, ``best_allele`` / ``HLA Allele``, ``tpm`` /
    ``Gene Expression``, ``gene_name`` / ``Gene Name``), plus ``ref_seq``/``seq`` and any built-in
    ``score``. Presentation is recomputed with ``store`` when
    ...
    with open(path, newline="", encoding="utf-8-sig") as fh:
        for rec in csv.DictReader(fh):
            pep = _first(rec, PEPTIDE_COLUMNS).upper()
            if not pep or not pep.isalpha():
                continue
            allele = _first(rec, ALLELE_COLUMNS)
            # Resolved by name from the shared ordered list, so a table annotated by
            # `mhcmatch genes` (`gene`) and one straight out of pVACseq (`Gene Name`) both drop in
            # here with no rename stage.
            gene = _first(rec, GENE_COLUMNS)
            try:
                _tpm = _first(rec, TPM_COLUMNS)
                tpm = float(_tpm) if _tpm else None
            except ValueError:
                tpm = None
            ...
            scored = allele
            if store is not None and (alleles or allele):
                bs = P.binder_score(store, pep, alleles=(alleles or [allele]), cls=cls)
                if bs:
                    # `binder_score` returns its list sorted by `binder_rank`, so `bs[0]` is the
                    # binder argmin already; presentation needs the explicit min or `allele_scored`
                    # would mean best-by-binder here and best-by-presentation in `rank_pairs`.
                    b = bs[0] if best_by == "binder" else min(bs, key=lambda x: x.presentation_rank)
                    pres = _neglog10(b.presentation_rank)
                    binder = _neglog10(b.binder_rank)
                    scored = b.allele
            ...
            nm = _ic50_of(rec)
            r = Ranked(peptide=pep, allele=allele, allele_scored=scored, gene=gene,
