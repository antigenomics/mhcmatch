...
def load_references(pmhc_dir=None, cls: str = "mhc1", with_self: bool = True,
    ...
    **~75 s for one class-II length, ~17 s for one class-I length.**
    ...
    for comp in COMPONENTS:
        src = {}
        if comp == "self" and cls == "mhc1":
            if not with_self:
                continue
            # `self` is the RECIPIENT's proteome, **at class I**. The fitted coefficients were
            # estimated with the human one, so a mouse run reports its mouse-self channel beside
            # that rather than silently substituting a differently-scaled feature into the same
            # weight. Class II reads a self ligandome through the deposit branch below, which is
            # also why a class-II run builds no proteome index at all.
            ...
            if comp == "self" and not with_self:
                continue
            rel = mimics.ref_path(comp, self_species, cls=cls)
...
def reference_species(species: str, comp: str, native: bool = False) -> str:
    ...
    **pathogen** fits do declare one, and they were fitted under this redirect -- ``C_corpus_self``
    ...
    cell (p = 0.24), which is what the ``I-Ab``-only thymic deposit above predicts.
...
def corpus_counts(pmhc_dir=None, cls: str = "mhc1", comp: str = "thymus", k: int = CORPUS_K,
    ...
    **It applies to the assayed deposits (``thymus``, ``viral``) and is a no-op for ``self``**, and
    ...
    if comp == "self" and cls == "mhc1":
        # **Class I only.** `C_corpus_self` is a fitted term of two shipped class-I artifacts, so
        # redefining its reference would be a model change rather than a definition, and the
        # class-I `self` channel stays the host proteome. Class II takes a self LIGANDOME through
        # the deposit branch below (`mimics.CLS_REFS`): there the proteome saturated the table --
        # 8,000 of 8,000 3-mer cells at ~111 M window mass -- and a near-uniform density cannot
        # discriminate, which is the whole job of the channel.
        cat = "self" if self_species == "human" else "self_mouse"
        per_len = {L: mimics.proteome_window_array(cat, L) for L in lengths}
    else:
        ...
        # deposit is split per species (thymus, and class-II self), `cls=` swaps it where the two
        # classes need a different object (class-II self and viral), and `species=` filters the
        # column where one file holds both (class-I viral). The class-II viral deposit is
        # pan-species and carries no such column, so the filter is a no-op there by construction.
        rel = mimics.ref_path(comp, self_species, cls=cls)
        peps = mimics.load_peptides(pmhc_dir, rel, cls, species=self_species)
        per_len = {L: np.array(_windows(peps, L), dtype=f"S{L}") for L in lengths}
    ...
    # lengths is one locus. A **proteome's** windows carry no peptide identity beyond the window
    # itself and there is no "assayed more often" to undo, so class-I `self` is still exempt; the
    # class-II self ligandome is a sample like any other deposit and is not.
    wmap: dict = {}
    if weights == "locus" and not (comp == "self" and cls == "mhc1"):
    ...
    for L, win in per_len.items():
        # The face is as wide as the class leaves it: class I deletes five anchors, class II's
        # floating core deletes four (P1/P4/P6/P9), and under `wildcard` nothing is deleted at all
        # so every window is `L` wide. A reference narrower than `k` contributes nothing and is
        # skipped rather than padded, either way.
        #
        # This read `L - 5` for both classes until 2026-09-20 and was harmless rather than right:
        # it only gates the skip, the divisor is the real window count, and `L - 5 < k <= L - 4`
        # has no solution over the class-II lengths 11-25 at `k = 3`. It would bite the moment
        # either the length range or `k` moved.
        width = L if mask == "wildcard" else L - (4 if cls == "mhc2" else 5)
        ...
        # Every class-II reference is now a set of LIGANDS -- thymus, the self ligandome and the
        # pan-species viral one -- so every one of them has a register to resolve, and none takes
        # the raw-k-mer path a proteome needed.
        if mask == "wildcard":
            F = V.copy()
...
def features(peptides, refs: dict, cls: str = "mhc1") -> list[dict]:
    ...
    **Batched per (component, channel, length), not per peptide.** The indices are already built per
    that key by :func:`load_references`, so a per-peptide :meth:`~seqtree.Index.search` asked the
    same index six questions per peptide -- one C++ entry per question, GIL held between them --
    where ``search_batch`` asks it once for every peptide of that length with the GIL released.
    Same index, same radius, same hits; the loop that was there is the one rule 2 forbids."""
    import math
    ...
    # channels rather than once per (component, channel) -- three components share one projection.
    proj: dict[int, list[int]] = {}
    masked: dict[int, dict] = {}
    for i, p in enumerate(peps):
        if all(c in AA for c in p):
            proj.setdefault(len(p), []).append(i)
            m = masks(len(p), cls, p)
            masked[i] = {ch: "".join(p[j] for j in m[ch]) for ch in CHANNELS}

    sp = {ch: SearchParams(max_subs=rad[ch], engine="seqtm") for ch in CHANNELS}
    for L, idxs in proj.items():
        for comp in COMPONENTS:
            for ch in CHANNELS:
                key = (comp, ch, L)
                if key not in refs:
                ...
                for i, hits in zip(idxs, index.search_batch(qs, sp[ch], 0)):
                    ...
                    if hits:
                        ...
                        out[i][f"nearest_{comp}_{ch}"] = {"subs": int(h.score), "peptide": pep,
...
# like the first one was. Measured on Aldan-3 2026-09-20, four class-I lengths: **63.9 s** of a
...
def _refs_key(cls: str, with_self: bool, self_species: str, lengths) -> str:
    ...
    # **The leading int is a schema epoch and it is hand-moved, for the reason `SCORER_EPOCH`
    # exists.** Every other component of this digest is pure data -- deposit names and sizes -- so
    # it cannot see a change in what the code does with them. 1 -> 2 is class-II `self` moving from
    # the host proteome to a self ligandome: same class, same species, same `with_self`, entirely
    # different reference, and an index cached under 1 would otherwise keep being served.
    h.update(f"2|{cls}|{int(with_self)}|{self_species}|{','.join(map(str, lengths))}|".encode())
    for comp in COMPONENTS:
        if comp == "self" and cls == "mhc1":
            if not with_self:
                continue
...
def _refs_dir(key: str):
    # `calibrate.cache_dir` directly, not via `proteome`: the proteome index used to live in a
    # sibling directory under the same root and this climbed out of it with `dirname`. There is no
    # proteome index any more -- one `seqtree.TextIndex` builds in 0.7 s -- but THIS cache stays,
    # because what it holds is masked-face projections, which a text index cannot answer.
    from .calibrate import cache_dir
    d = cache_dir()
    if d is None:
        return None
    d = os.path.join(d, "mimicry_refs", key)
    try:
...
def _unlink(path: str) -> None:
    """Best-effort remove of a temp file whose write failed.

    **This was `from .proteome import _unlink` until 2026-09-20, and `proteome` stopped having one
    in `d929439`** -- the commit that deleted the proteome disk cache wholesale. The import sits
    inside `_refs_to_disk`'s own `except Exception`, so it raised `ImportError` on every call and
    was swallowed: the function wrote nothing, `manifest.npz` was never created, and
    `_refs_from_disk` therefore missed every time. Silent, and worth 63.9 s of a 200 s
    `mhcmatch mimicry` task per process. A blanket `except` around an import is how a deleted
    symbol becomes a performance regression nobody can see."""
    try:
        os.unlink(path)
    except OSError:
        pass
