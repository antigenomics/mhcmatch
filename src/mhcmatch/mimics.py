...
__all__ = ["KINDS", "DEFAULT_REFS", "SPECIES_REFS", "CLS_REFS", "ref_path",
           "PROTEOME_REFS",
...
#: :data:`SPECIES_REFS`.
#:
#: Two of the three class-II references are a different object from their class-I namesakes, and
#: both differences are forced by what the reference has to argue:
#:
#: * ``self`` at class I is the host **proteome** and stays one -- ``C_corpus_self`` is a fitted
#:   term of two shipped class-I artifacts, so redefining it would be a model change. At class II
#:   the proteome saturates the table (8,000 of 8,000 3-mer cells, ~111 M window mass), and a
#:   near-uniform density cannot discriminate. The class-II channel therefore reads a self
#:   **ligandome** -- IEDB class-II ligands whose source protein is the host's own, 421,174 human
#:   and 25,314 mouse peptides -- which is what "presented" means and what tolerance is about.
#: * ``viral`` is **pan-species at class II**. The deposit carries no ``mhc_species`` column, so
#:   :func:`load_peptides` keeps every row whichever species asks. Splitting it left the mouse cell
#:   on 464 peptides against human's 14,468; a viral k-mer density is not species-specific the way
#:   a proteome is, and pooling deepens the mouse reference 31x while moving human 14,468 -> 14,645.
#: * ``thymus`` at human class II is the **whole displayed proteome, not the thymus** -- every
#:   benign tissue the HLA Ligand Atlas sampled on a class-II molecule, 132,818 peptides against
#:   the thymic fraction's 27,987. The channel was never thymic: measured across the atlas's 29
#:   tissues on 7,948 human class-II T-cell rows of which 5,150 are immunogenic, lung leads at
#:   contrast AUROC 0.5496 (95% CI 0.5365-0.5627) and thymus is second at 0.5475
#:   (0.5345-0.5606) -- the same result class I gives, where thymus is 6th of 29. The slot keeps
#:   the name ``thymus`` because ``C_corpus_thymus`` is a released column name and renaming it
#:   would misname the model a reader downloads; every reader-facing label says *displayed self*.
#:   Mouse class II is untouched by this and still reads its own thymic deposit.
CLS_REFS = {
    ("self", "mhc2", "human"): "ligandome/self_ligandome_mhc2_hsa.tsv.gz",
    ("self", "mhc2", "mouse"): "ligandome/self_ligandome_mhc2_mmu.tsv.gz",
    ("viral", "mhc2", "human"): "ligandome/viral_ligandome_mhc2_pan.tsv.gz",
    ("viral", "mhc2", "mouse"): "ligandome/viral_ligandome_mhc2_pan.tsv.gz",
    ("thymus", "mhc2", "human"): "ligandome/hla_ligand_atlas_mhc2_presented.tsv.gz",
}


def ref_path(category: str, species: str = "human", refs: dict | None = None,
             cls: str | None = None) -> str:
    """The compendium path for ``category`` in ``species``, for ``cls`` when it matters.

    Resolution order is most specific first: :data:`CLS_REFS` (``category``, ``cls``, ``species``),
    then :data:`SPECIES_REFS` (``category``, ``species``), then the species-agnostic
    :data:`DEFAULT_REFS`. Only a deposit that is physically split needs an entry above the last.

    ``cls=None`` skips the class layer, which is what a caller with no class in hand wants; it is
    not the same as ``cls="mhc1"``, because a class-I-only override would then be reachable from a
    call that never named a class.
    """
    if species not in _SPECIES:
        raise ValueError(f"species must be one of {sorted(_SPECIES)}, got {species!r}")
    if cls is not None:
        over = CLS_REFS.get((category, cls, species))
        if over is not None:
            return over
...
def scan(binders, self_set, foreign_sets, cls="mhc1", max_subs=2, near_subs=2, self_name="thymus",
    ...
    **``evalue`` defaults to ``False``, the fast path, from 1.20.0 --- it used to default to
    ``True``.** On the fast path the search runs through :func:`neighbours` (one batched, threaded
    index query per category and length) instead of one :func:`find_mimics` call per binder, which
    is **4,300x** the throughput on measured identical answers: every field except ``e_value`` and
    ``n_hits`` is identical, and ``tests/test_mimics.py`` pins that equality.

    The default moved because the expensive path was the one you got without asking, while the
    docstring recommended the other -- and `find_mimics` builds its index per call, so the cost is
    quadratic in the obvious usage. **What you give up is real and not silent:** ``e_value`` comes
    back ``nan`` and ``n_hits`` becomes the near-hit count, so a caller reading significance sees
    ``nan`` rather than a plausible wrong number. Pass ``evalue=True`` when the per-allele
    presentation-aware significance *is* the question; it now honours ``threads``, which it
    ...
    for pep, allele in binders:
        if evalue:
            res = find_mimics(pep, self_set, bacterial_sets=foreign_sets, cls=cls,
                              max_subs=max_subs, threads=threads)
