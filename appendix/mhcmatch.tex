...
\begin{figure}[ht]\centering
  ...
  \begin{subfigure}{0.49\linewidth}\includegraphics[width=\linewidth]{pockets_mhc2_mouse.pdf}
    \caption{MHC-II, mouse}\end{subfigure}
  \caption{DPI-pruned mutual-information relevance $w_{j,p}$ of each groove pseudosequence position
  $p$ (x) for each anchor pocket $j$ (rows), per class and species. After data-processing-inequality
  pruning~\cite{Margolin2006} each pocket keeps only its \emph{direct} positions (MHC-I P$\Omega$
  $\to\sim$19, P2 $\to\sim$7--8, disjoint); MHC-II concentrates on $\beta_1$. \texttt{bench/make\_figures.py}.}
  \label{fig:pockets}
\end{figure}
...
generalization. \emph{Metrics} are top-1 and top-5 accuracy and per-rarity \emph{recovery@5} (the
...
is the negative class for the eventual multi-class (allele $+$ non-binder) confusion-matrix
evaluation. \emph{Comparison}: against NetMHCpan-4.1 and NetMHCIIpan-4.0
