...
as seen on a representative complex in Fig.~\ref{fig:structural}---and
...
\begin{figure}[ht]\centering
  \includegraphics[width=0.72\linewidth]{structure3d.png}
  \caption{The learned pockets on a real structure: a top-down view into the HLA-A*02:01 groove with
  the influenza M1 peptide GILGFVFTL bound (PDB 1oga, oriented by \texttt{tcren orient}; histo.fyi
  style---pale translucent surface + cartoon, peptide as sticks). Each groove residue is coloured by
  the peptide anchor it lies closest to, using the same pocket palette as the per-pocket weights:
  P1 (orange), P2/B-pocket (blue), P3 (light blue), P$\Omega{-}1$ (green), P$\Omega$/F-pocket
  (vermillion). The buried primary anchors P2 and P$\Omega$ are labelled; their side chains plug into
  the blue B-pocket and vermillion F-pocket that the structural contacts (\S\ref{sec:pockets}) and the
  pruned MI weights (Fig.~\ref{fig:pockets}) both single out. \texttt{bench/structure3d.py} (PyMOL).}
