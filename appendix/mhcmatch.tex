...
($\le 30$ peptides) from $0.82$ to $0.94$ on the high-confidence ($\ge2$-publication) set while
leaving \emph{frequent} alleles unchanged ($0.975\!\to\!0.975$); on the noisier full set the
rare-allele gain is $+0.06$ and frequent alleles are never harmed (Fig.~\ref{fig:diffauc}). Stats are
computed fairly: held-out peptides are excluded from \emph{every} allele's training set, so an
identical co-presented copy cannot leak into a rare allele's score through diffusion (this lowers the
raw baseline and \emph{widens} the honest gain). The richer pocket footprint matters: with the two
...
\paragraph{Allele prediction (top-$k$, promiscuity-aware).} Cast as the reverse problem---rank all
panel alleles for a held-out peptide and ask whether a true restricting allele lands in the
top-$k$---the gain is large where it matters (\texttt{bench/tune\_diffusion.py}, human, per-allele
hold-out, fair as above). Because peptides are promiscuous we score \emph{recovery@5} (the fraction
of a peptide's true alleles ranked in the top~5) split by allele rarity. For MHC-I the diffusion
lifts \emph{rare}-allele recovery@5 from $0.50$ to $0.88$ (shortlist) and $0.29$ to $0.47$ (full
tier), at a small frequent-allele cost ($\approx-0.03$) and with overall top-5 $\approx0.95$; a grid
sweep selects $h=2,\tau=10$ with the BLOSUM metric (marginally over identity) as optimal. Class~II
remains hard (recovery@5 $\approx0.14$, near-neutral diffusion): its open groove, promiscuity, and
one-pass register cap the anchor model---a target for the structure-based extension. Top-1 is
\emph{not} the right yardstick here (promiscuity makes several alleles correct); top-5/recovery@5 is.

\begin{figure}[ht]\centering
  \includegraphics[width=0.8\linewidth]{diffusion_auc.pdf}
  \caption{Held-out rank AUC for rare ($\le30$ peptides) and frequent ($\ge200$) alleles, without
  (raw, grey) and with (blue) pseudosequence diffusion, allele-split. MHC-I on the
  $\ge2$-publication set, MHC-II on the full set. Diffusion lifts rare alleles and leaves frequent
  alleles unchanged in both classes. \texttt{bench/bench\_diffusion.py}.}
  \label{fig:diffauc}
\end{figure}
...
\begin{tikzpicture}[>=stealth,
  ...
  \node[hyp] (nb) {groove neighbours\\$\theta_{b,j},\,K_j(a,b),\,n_b$};
  \node[latent, right=20mm of nb] (m) {$m_{a,j}$};
...
\end{tikzpicture}
\caption{Graphical model behind the diffusion shrinkage. The kernel-weighted neighbour mean
$m_{a,j}$ and the strength $\tau$ form a Dirichlet \emph{prior} on the allele's pocket-$j$ residue
distribution $\theta_{a,j}$; the $n_a$ observed anchor residues $\sigma_{i,j}$ (shaded) are the
\emph{likelihood}. The posterior mean is the shrinkage estimator~\eqref{eq:shrink-tau}: rare alleles
borrow from groove-similar neighbours, well-sampled alleles do not.}
\label{fig:bayes}
\end{figure}
...
log-odds is used as the binder \emph{gate}---it scores every requested allele, surfacing rare alleles
with no signature neighbours---while the vote fraction~(Def.~\ref{def:reverse}) still \emph{ranks},
since per-peptide anchor likelihood is diluted for broad, well-sampled alleles and is not itself a
calibrated cross-allele ranking statistic.
...
An empirical counterpart (Fig.~\ref{fig:promiscuity-shared}, all four class\,$\times$\,species panels)
builds the same network from \emph{observed} co-presentation---edges are the overlap coefficient
...
\begin{figure}[ht]\centering
  ...
  \begin{subfigure}{0.49\linewidth}\includegraphics[width=\linewidth]{promiscuity_shared_mhc1_mouse.pdf}
  ...
  \begin{subfigure}{0.49\linewidth}\includegraphics[width=\linewidth]{promiscuity_shared_mhc2_mouse.pdf}
    \caption{MHC-II, mouse}\end{subfigure}
  \caption{\emph{Observed} co-presentation networks (shared-epitope overlap coefficient; thin
  $\ge0.25$, bold $\ge0.5$; greedy-modularity communities), all four class\,$\times$\,species panels.
  Communities are coarser than the predicted groove network (Fig.~\ref{fig:promiscuity}) because
  promiscuous peptides bridge supertypes. \texttt{bench/promiscuity\_graph.py}.}
  \label{fig:promiscuity-shared}
\end{figure}
...
(Fig.~\ref{fig:pockets}, all four panels). For MHC-I (human) the two primary pockets read
\emph{disjoint} groove positions---P2/B-pocket concentrates near positions $7$--$8$, P$\Omega$/F-pocket
near $18$--$21$---directly recovering the structural pocket layout from presented-peptide statistics
alone. For MHC-II the relevance sits on the polymorphic $\beta_1$ segment and is near-zero on the
largely monomorphic $\alpha_1$ (sharpest for DR, whose DRA $\alpha$ chain is invariant). The kernel
is thus not one global allele distance but a sum of pocket-specific distances, each restricted to the
residues lining its pocket. Mouse panels are data-limited ($8$--$13$ alleles), so their weights are
noisier but show the same gross localization. (Class-II alleles are keyed by the $\alpha$--$\beta$
pair, cores from the register trick; its diffusion gain is small but safe, $0.72\!\to\!0.74$ AUC.)

\begin{figure}[ht]\centering
  \begin{subfigure}{0.49\linewidth}\includegraphics[width=\linewidth]{pockets_mhc1_human.pdf}
    \caption{MHC-I, human}\end{subfigure}\hfill
  \begin{subfigure}{0.49\linewidth}\includegraphics[width=\linewidth]{pockets_mhc1_mouse.pdf}
    \caption{MHC-I, mouse}\end{subfigure}

  \begin{subfigure}{0.49\linewidth}\includegraphics[width=\linewidth]{pockets_mhc2_human.pdf}
    \caption{MHC-II, human}\end{subfigure}\hfill
  \begin{subfigure}{0.49\linewidth}\includegraphics[width=\linewidth]{pockets_mhc2_mouse.pdf}
    \caption{MHC-II, mouse}\end{subfigure}
  \caption{Learned mutual-information relevance $w_{j,p}$ of each groove pseudosequence position $p$
  (x) for each anchor pocket $j$ (rows), per class and species. Different pockets read different
  groove positions (MHC-I P2 vs P$\Omega$ are disjoint); MHC-II concentrates on $\beta_1$.
  \texttt{bench/make\_figures.py}.}
  \label{fig:pockets}
\end{figure}
...
(FDR)~\cite{Benjamini1995}. \texttt{scan\_protein} reports per-window binders; the grid-level
correction is a calibration step.
...
We evaluate as a per-(peptide, allele) binary task. \emph{Splits} are by allele \emph{and} by
peptide: held-out alleles test cross-allele generalization (and the diffusion model of
\S\ref{sec:diffusion}), held-out peptides within an allele test within-allele recall; peptide
duplicates are collapsed before splitting, and crucially a held-out peptide is excluded from
\emph{every} allele's training set (not just its own), so an identical co-presented copy cannot leak
into a score---in particular through cross-allele diffusion. Equivalently, all matching counts are
\emph{punctured}: exact-identity hits are dropped, never rewarded (mirroring the $n^{>}$ counts of
the \texttt{seqtree} appendix). This lowers raw baselines but is the only fair measure of
generalization. \emph{Metrics} are ROC-AUC,
PR-AUC against the prevalence baseline, top-1 accuracy, and a real-vs-random noise-rejection AUROC
(the non-binder filter). \emph{Comparison}: against NetMHCpan-4.1 and NetMHCIIpan-4.0
