...
\begin{equation}\label{eq:kernel}
  d_j(a,b)=\sum_{p=1}^{34} w_{j,p}\,\one[s_{a,p}\neq s_{b,p}],\qquad
  K_j(a,b)=\exp\!\big(-d_j(a,b)/h_j\big),
\end{equation}
a weighted-Hamming kernel with bandwidth $h_j>0$ (ambiguous positions $\texttt{X}$ are skipped).
Setting $w_j\equiv 1$ recovers a single un-factored kernel.
...
held-out rank AUC, $\tau=10$, $h=2$, the per-allele score being the anchor log-odds over the N/C
pocket footprint $\{$P1,P2,P3,P$(\Omega{-}1)$,P$\Omega\}$) the diffusion lifts the AUC of \emph{rare}
MHC-I alleles ($\le 30$ peptides) from $0.85$ to $0.94$ on the high-confidence ($\ge2$-publication)
set while leaving \emph{frequent} alleles unchanged ($0.975\!\to\!0.975$); on the noisier full set
the rare-allele gain persists ($+0.06$) and frequent alleles are never harmed
(Fig.~\ref{fig:diffauc}). The richer pocket footprint matters: with the two primary anchors alone
($\{$P2,P$\Omega\}$) the full-set rare gain is only $+0.003$, whereas the auxiliary pocket-proximal
positions make per-allele estimation sparser---so diffusion has more to rescue and the gain grows to
$+0.06$. The data-rescue is safe: it never lowers a well-sampled allele's AUC.
...
\S\ref{sec:diffusion}), held-out peptides within an allele test within-allele recall; clonotype/
peptide duplicates are collapsed before splitting to prevent leakage. \emph{Metrics} are ROC-AUC,
