...
($\le 30$ peptides) from $0.85$ to $0.95$ on the high-confidence ($\ge2$-publication) set while
leaving \emph{frequent} alleles unchanged ($0.975\!\to\!0.975$); on the noisier full set the
rare-allele gain is $+0.05$ and frequent alleles are never harmed (Fig.~\ref{fig:diffauc}). Exclusion
...
\paragraph{Allele prediction (top-$k$, promiscuity-aware).} Cast as the reverse problem---for each
held-out pMHC $(p,a)$ rank all panel alleles for $p$ and ask whether the held-out allele $a$ is
recovered in the top-$k$ (\texttt{bench/tune\_diffusion.py}, human, per-pMHC hold-out). Because
peptides are promiscuous we score \emph{recovery@5} split by allele rarity. For MHC-I the diffusion
lifts \emph{rare}-allele recovery@5 from $0.53$ to $0.88$ (shortlist) and $0.29$ to $0.47$ (full
tier), at a small frequent cost ($\approx-0.02$). Here the \emph{ranker} is the diffused anchor
log-odds itself, not the neighbour vote: on a novel peptide the vote tally has few same-allele
signature neighbours, so vote-first ranking buries the true allele (top-5 $0.87$, rare recovery@5
$0.25$), whereas the diffused log-odds scores every allele directly (top-5 $0.95$, rare recovery@5
$0.88$). A grid sweep selects $h=2,\tau=10$, BLOSUM (marginally over identity), as optimal. Class~II
