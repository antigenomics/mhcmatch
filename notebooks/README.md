...
| [`10_cassette_select_and_score.py`](10_cassette_select_and_score.py) | `mhcmatch.cassette` | the two operations on a whole published corpus: `select` on the 46 NCI GI patients held out of the EPIC fit, `score` on the 1,631 units two trials manufactured. That `select` wins on `H` (0.219 vs 0.127) and a sort wins on `yield` (1.044 vs 0.768), because they maximise different things; that one offset per donor pins every pool mean to 0.060162602 (sd 2.4e-17) and collapses the `yield` spread from 0.0789 to exactly 0; that `lam` needs no shared calibration at all; and the profile coupling, which reads the score's own decomposition -- `aggregate_terms` rows summing to `aggregate_score` to 4.4e-16, an off-diagonal coupling of 0.164 (sd 0.234) over the 46 pools, and `epic_axes` refusing to whiten a twelve-row pool against its own covariance | ~95 s |
