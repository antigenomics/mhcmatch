...
- **`cassette.selectivity_delta` and `select(selectivity=w, ...)` / `--selectivity`** -- a stated
  ...
  model fits `expr_lvl` +0.3704 and `expr_norm` **+0.4950** -- both positive, the second the largest
  coefficient in the model -- because it was fitted on *will this respond*, and a gene transcribed
