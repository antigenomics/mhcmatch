...
- **Re-baselined:** `bench/results/register_em_mhc2.md`, `compare_mhc2_human_hard_ligandbg.md`,
  ...
  stratum membership; the frequent row is stable across that (0.827 → 0.830).
- **Still open:** the MHC-II binder gate remains a length detector. Marginalizing halves the
  inflation (random peptides 9mer→21mer: +4.44 nats → **+2.28**) but a Jensen residual remains, so an
  absolute binder call still needs a length-matched `%rank`. See `ROADMAP.md` §6c.
